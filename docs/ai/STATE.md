...
| Gemini ACP artifact reporting fix — issue_comment path (Issue #174) | **IMPLEMENTED / VERIFIED** | Kilo | Fixed the remaining `issue_comment` execution path defect where `gemini-acp-report.json` persisted raw Gemini CLI/Markdown output instead of the structured ACP envelope. Changes to `.github/workflows/main.yml`: (1) Removed the "Persist Gemini result as artifact" step that wrote raw `steps.gemini_run.outputs.summary` to `gemini-acp-report.json`; (2) Generalized the "Prepare ACP report payload" step (renamed from "Prepare callback payload") to run for both `workflow_dispatch` and `issue_comment` paths (`if: always()` without event_name gate); (3) Added trigger-path conditional — for `issue_comment`, derives `task` from `steps.request_comment.outputs.request`, `repository` from `github.repository`, `base_branch` from `github.ref_name`; `request_id` and `kilo_execution_id` genuinely unavailable → `request_id` set to `null` via jq `if $request_id == "" then null else $request_id end`; (4) Removed the issue_comment-only artifact upload step; (5) Unified artifact upload to a single `if: always()` step covering both trigger paths. Preserved: `@gemini-cli` issue-comment triggering, Gemini CLI execution, structured callback (workflow_dispatch-only Render callback preserved), ACP authorization boundaries, recursion-prevention, artifact name (`gemini-acp-report`), artifact file (`gemini-acp-report.json`), workflow ordering, secrets boundaries. 29 tests pass (workflow-expression.test.js). (TASK-KILO-GEMINI-ACP-ARTIFACT-ISSUE-COMMENT-FIX-002) |
